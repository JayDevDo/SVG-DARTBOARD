...
//==============================================================================
function getDartsNeeded( score ){let scoreInfo = DFC_STATE.scores[score]; if( !scoreInfo ){return 99;} return scoreInfo.DARTSNEEDED;}
...
function getScoringSegments(){
	// Only scoring segments. Excluding skinny singles avoiding duplication and Outer Ring which has SegVal = 0.
	let retArr = [];
	for( let segment of DFC_STATE.segments ){
		if( segment.SegInRad !== 2 && segment.SegVal > 0 ){retArr.push( segment );}
	}
	return retArr;
}
...
function getFinishDoubleSegments(){
	...
	let retArr = [];
	for( let segment of DFC_STATE.segments ){
		if( segment.SegMulti === 2 ){retArr.push( segment );}
	}
	return retArr;
}

//==============================================================================
function getFinishSegmentForScore( score ){
	for( let segment of getFinishDoubleSegments() ){
		if( segment.SegVal === score ){return segment;}
	}
	return null;
}
...
function getSetupRouteDiff( segments, scoreBefore, scoreAfter ){
	if( segments.length === 1 && getDartsNeeded( scoreBefore ) === 2 ){
		let finishSegment = getFinishSegmentForScore( scoreAfter );
...
function addOneDartSetupRoutes( routes, routeKeys, score ){
	let setupSegments = getScoringSegments();
